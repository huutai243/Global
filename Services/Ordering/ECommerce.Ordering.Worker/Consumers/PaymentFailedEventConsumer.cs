...
public sealed class PaymentFailedEventConsumer(ECommerceDbContext dbContext, ILogger<PaymentFailedEventConsumer> logger)
{
    public async Task HandleAsync(PaymentFailedEvent message, CancellationToken cancellationToken = default)
    {
        if (await HasProcessedAsync(nameof(PaymentFailedEvent), message.OrderId, cancellationToken))
        {
            return;
        }
        ...
        if (order is not null && order.Status == OrderStatus.PendingPayment)
        {
            ...
            AddOutboxMessage(new ReleaseInventoryReservationCommand(order.Id, order.CustomerId, message.Reason, DateTime.UtcNow));
            AddOutboxMessage(new OrderCancelledEvent(order.Id, order.CustomerId, message.Reason, DateTime.UtcNow));
        }

        MarkProcessed(nameof(PaymentFailedEvent), message.OrderId);
        await dbContext.SaveChangesAsync(cancellationToken);
        await transaction.CommitAsync(cancellationToken);

        logger.LogInformation("Ordering handled payment failure for order {OrderId}", message.OrderId);
    }
